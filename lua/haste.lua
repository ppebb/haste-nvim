...
function M.upload()
    ...
    if not handle then
        vim.notify(string.format(" Haste: Failed to spawn curl (%s)", pid))
    end

    local function onstdout(err, data)
        if err then
            vim.notify(string.format(" Haste: stdout - err: %s", err))
        elseif data then
            local index = string.find(data, "key")
            local url = _config.url .. "/" .. string.sub(data, index + 6, -3)
            if index ~= nil then
                vim.notify(string.format(" Haste: Your document is %s", url))
                if _config.setclip then
                    vim.schedule(function() vim.fn.setreg("+", url) end)
                end
            -- else
                -- vim.notify(string.format(" Haste: stdout - data: %s", data))
            end
        end
    end
    ...
    local function onstderr(err, data)
        if err then
            vim.notify(string.format(" Haste: stderr - err: %s", err))
        -- elseif data then
            -- vim.notify(string.format(" Haste: stderr - data: %s", data))
        end
    end
